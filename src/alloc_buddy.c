...
{
  int level;
  for (level = 0; max_bytes(level + 1) >= size && level < 9 - 1;
       level++)
    continue;
  return level;
}
...
# 147 "alloc_buddy.c"
  ...
  else if (get_buddy_size(cp) == level)
  {
    if (get_free(cp))
    {
      if (max_bytes(level + 1) < size || level + 1 == 9)
      {
        set_free(cp, 0);
        return sp + 1;
      }
